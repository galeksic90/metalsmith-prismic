...
function plugin(config) {
  ...
  return function(files, metalsmith, done) {
    ...
    function retrieveContent(err, api) {
      if (err) {
        ...
        for (var currentRef in api.data.refs) {
          if (
            (!release || release.toLowerCase() == "master") &&
            api.data.refs[currentRef].isMaster
          ) {
    ...
    function retrieveContentForFile(fileName, callback, ctx) {
      ...
      if (file.prismic) {
        ...
        for (var queryKey in file.prismic) {
          ...
          var lang = file.prismic[queryKey].lang;
          ...
          if (collectionMetaData != null) {
            if (collectionQuery != null) {
              throw new TypeError(
                "Only one query can be designated the collection query. Please update the configuration settings appropriately."
              );
            } else if (
              typeof collectionMetaData != "boolean" &&
              file.prismic[queryKey].collection.fileExtension == null
            ) {
              throw new TypeError(
                "The file extension to use when generating the files for the collection must be specified."
              );
            } else if (
        ...
        each(
          ...
          function(currentQueryKey, queriedAndProcessedCallback) {
            debug(
              "%s: api.form(\"%s\").query('%s').ref(ctx.ref).submit()",
              queryKey,
              formName,
              queryString
            );
            ...
            function queryNextPage() {
              ctx.api
                .form(queryKeys[currentQueryKey].formName)
                ...
                .lang(queryKeys[currentQueryKey].lang)
                .fetchLinks(queryKeys[currentQueryKey].fetchLinks)
                .ref(ctx.ref)
                .submit(prismicCallback);
            }
    ...
    function processRetrievedContent(
      content,
      filePrismicMetadata,
      queryKey,
      ctx
      ...
      if (content.results != null && content.results.length > 0) {
        ...
        for (var i = 0; i < content.results.length; i++) {
          ...
          if (
            content.results[i].fragments != null &&
            Object.keys(content.results[i].fragments).length > 0
          ) {
            for (var fragmentFullName in content.results[i].fragments) {
              // strip the document type from the fragment name
              var fragmentName = fragmentFullName.substr(
                fragmentFullName.lastIndexOf(".") + 1
              );

              // process fragments recursively
              var fragment = content.results[i].fragments[fragmentFullName];
              result.data[fragmentName] = processSingleFragment(
                fragment,
                ctx,
                queryMetadata
              );
    ...
    function processSingleFragment(fragment, ctx, queryMetadata) {
      ...
      // Add child fragments
      if (
        fragment instanceof Prismic.Fragments.DocumentLink &&
        fragment.document.data
      ) {
        // from document links
        fragmentObject.children = _.mapObject(
          fragment.document.data[fragment.type],
          function(subFragment) {
            if (subFragment.type) {
              return processSingleFragment(subFragment, ctx, queryMetadata);
            } else {
    ...
    function generateCollectionFiles(fileName, collectionQuery, callback, ctx) {
      if (collectionQuery != null) {
        ...
        var newFiles = {};
        var fileExtension =
          file.prismic[collectionQuery].collection.fileExtension;
        var fileSuffix =
        ...
        for (var i = 0; i < file.prismic[collectionQuery].results.length; i++) {
          ...
          newFile.prismic[collectionQuery].results = [
            file.prismic[collectionQuery].results[i]
          ];
          ...
          newFiles[
            (
              ctx.linkResolver(file.prismic[collectionQuery].results[i]) +
              fileSuffix
            ).replace(/^\//g, "")
          ] = newFile;
        }
  ...
  function checkProperty(object, property) {
    if (object[property] == null) {
      throw new TypeError(
        "Missing property '" +
          property +
          "'. Please update the configuration settings appropriately."
      );
    }
  }
}
